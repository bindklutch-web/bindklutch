// js/supabase.js
// Powers dynamic content on index.html (domains grid + featured scenarios).
// Uses the global `window.db` client created in js/db.js — does NOT create its own.
//
// REQUIRES (load these before this file on index.html):
//   1. supabase-js CDN
//   2. js/db.js   (sets window.db)
...
;(async function () {
  ...
  const db = window.db
  if (!db) {
    console.error('[supabase.js] window.db is not defined. Load js/db.js before this file.')
    return
  }
